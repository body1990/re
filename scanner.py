...
MAX_MARKET_CAP        = None          # e.g. 500_000_000 to skip big caps
NOTIFY_COOLDOWN_HOURS = 12
HISTORY_RETENTION_H   = 30            # keep ~30h of snapshots per coin
SNAPSHOT_TARGET_H     = 24            # compare against ~24h ago
SNAPSHOT_TOLERANCE_H  = 3             # accept 21h-27h old snapshot
...
def save_state(state: dict) -> None:
    with open(STATE_PATH, "w") as f:
        json.dump(state, f, separators=(",", ":"))  # compact, smaller diffs
...
def volume_around(state: dict, coin_id: str,
    ...
    """Return (volume, actual_age_hours) of snapshot closest to target within tolerance."""
...
def run_scan():
    ...
    for coin in coins:
        ...
        # Always snapshot so history grows for every coin
...
def main():
  send_telegram("✅ test ping")
    ...
    for h in hits[:10]:  # cap per-run spam at 10
